...
\paragraph*{}Using the RSS and TSS metric, we can compute the $R^2$ statistic for the linear regression fit given the two metrics.
\begin{equation}
\label{R^2}
R^2 = 1 - \frac{RSS}{TSS} = 1 - \frac{20}{120} = \frac{5}{6}
\end{equation}
Know that $R^2 = r^2$ which is the correlation between $X$ and $Y$, thus $Cor(X,Y) = r = \pm\sqrt{R^2} = \pm\sqrt{\frac{5}{6}}$.
Finall, from ISL, 3.18, we know how the correlation coefficient relates to the Covariance, and the two variances:
\begin{equation}
\label{Cor}
Cor(X,Y) = \frac{Cov(X,Y)}{\sqrt{\sigma^2_X}\sqrt{\sigma^2_Y}}
\end{equation}
Thus we solve for the covariance between $X$ and $Y$:
\begin{equation}
\label{Cov}
Cov(X,Y) = Cor(X,Y)\sqrt{Var(X)}\sqrt{Var(Y} = \pm\sqrt{\frac{5}{6}(15)(20)} = \sqrt{250}
\end{equation}
\paragraph*{}Thus for RSS = 20 , TSS = 120, Var(X) = 15 and Var(Y) = 20, the resulting covariance, Cov(X,Y) = $\pm\sqrt{250}$.

\subsection*{2. Repeat if RSS=50 and TSS=40. Discuss the result.}
\paragraph*{}Repeat the same process as above, We can compute $R^2 = -1/4$ which onto it's own creates a problem! $R^2$ is bound on the inter val $[0,1]$ so a negative number certain raises some issues. Looking at the respective definitions of RSS and TSS, it seems impossible for this to occur in practice, thus I would attribute this to a numerical error, switching the two metrics or producing a fit so bad, we've broken statistics so much that we've pushed ourselves into the complex plane for the correlation coefficient. However, continuing the calculation would show that Cov(X,Y) = $\frac{1}{2}i\sqrt{300}$, which again, is physically unrealistic, uninterpretable and holds no physical meaning.
...
I'll give this one my best shot. but I'm not sure!
\paragraph*{}The Maximum likelihood solution is obtained my minimizing the objective function:
\begin{equation}
\hat{L}(\theta;x) = \frac{1}{n} \sum_{i=1}^{n} \ln \Big[ f(x;\theta) \Big]
\end{equation}
We want $f(x;\theta)$ to be of the form $\hat{\theta}_0 + \hat{\theta}_1x$ with an associate error according to the Laplace distribution is given by:
\begin{equation}
\epsilon = \frac{1}{2b}e^{-\frac{|x-\mu|}{b}}
\end{equation}
\paragraph*{}Thus, putting this into the objective function:
\begin{equation}
\hat{L}(\theta;x) = \frac{1}{n} \sum_{i=1}^{n} \ln \Big[ \hat{\theta}_0 + \hat{\theta}_1x + \frac{1}{2b}e^{-\frac{|x-\mu|}{b}} \Big]
\end{equation}
Truthfully, I'm not sure how to go about this one! I was hoping to be able to use the natural log of $e$ to just keep the exponent factor, and hopefully use that in combination with the remaining terms, along with the average parameter to quantify the Least absolute deviation. Unfortunately, I can't seem to get the math to work out right now, though I do hope to see the solution to this problem, it seems very interesting!
...
Using Quantile regression with $\tau = \frac{1}{2}$
